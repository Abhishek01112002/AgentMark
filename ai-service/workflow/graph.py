...
def copywriter_node(state: CampaignState) -> dict:
    ...
    try:
        # Copy revision: clear ALL downstream outputs (hooks, image, review).
        # Creative Hooks and Image Prompts both use copy_output as input context,
        # so they MUST re-run when copy changes to keep the pipeline consistent.
        if is_targeted_for_revision:
            logger.info("   🧹 Revision: Clearing downstream outputs (hooks, image, review)...")
            state.creative_hook_matrix_output = None
            state.image_output = None
            state.review_output = None
        ...
        return {
            ...
            "review_output": getattr(updated_state, "review_output", None),
