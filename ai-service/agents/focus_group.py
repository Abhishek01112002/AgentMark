...
async def _run_single_persona_critique(
    ...
    system_prompt = (
        ...
        "1. VISCERAL REACTION: First, output your immediate visceral emotion (e.g. Anxiety, Relief, Skepticism).\n"
        "2. INTERNAL MONOLOGUE: State what you are literally thinking to yourself in the first 3 seconds.\n"
        "3. PERCEIVED FRICTION: Evaluate how much effort it takes to click/buy/sign up. If friction outweighs reward, you bounce.\n"
        "4. Your attitude, tone, and skepticism level are strictly defined by your Psychometric Profile and Cognitive Bias. Act irrationally if your bias dictates it (e.g. Loss Aversion).\n"
        "5. If a buying barrier is unaddressed or triggers your specific bias, penalize the Trust score severely.\n"
        "6. In the 'clash_quote' field, quote the EXACT sentence from the copy that triggered your reaction.\n"
        f"{res_ctx_str}"
    )
...
async def _run_analyst_synthesis(
    ...
    """
    # 1. Dynamic Threshold Scoring (Context-Aware Math instead of Negativity Bias)
    ...
    is_high_risk = any("loss aversion" in getattr(p, "dominant_cognitive_bias", "").lower() for p in personas)
    if is_high_risk:
        # Consensus Model: Softened veto weight to prevent token-wasting infinite revision loops (0.25 min + 0.75 avg)
        overall_score = int(0.25 * min_score + 0.75 * avg_score)
    else:
    ...
    system_prompt = (
        ...
        f"- The overall score (already calculated dynamically based on product risk profile as {overall_score})\n"
        ...
        "POLARIZATION CHECK: If personas contradict each other, do NOT suggest a watered-down compromise. Explicitly flag it as a 'Polarizing Element' and suggest an opinionated rewrite leaning into the primary target.\n"
