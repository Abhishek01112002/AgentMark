...
def human_approval_node(state: CampaignState) -> CampaignState:
    ...
    if state.review_output:
        try:
            ...
            print(f"   Status: {review_data.get('status', 'N/A')}")
            
            # Show individual scores
            print("\n   Agent Scores:")
            print(f"   - Research:  {review_data.get('research_review', {}).get('score', 'N/A')}/100")
            print(f"   - Strategy:  {review_data.get('strategy_review', {}).get('score', 'N/A')}/100")
            print(f"   - Copy:      {review_data.get('copy_review', {}).get('score', 'N/A')}/100")
            print(f"   - Image:     {review_data.get('image_review', {}).get('score', 'N/A')}/100")
        except:
            pass
    ...
    print("    'feedback': 'Optional message',")
    print("    'revision_target': 'copywriter'  # Required if action=reject")
    print("  }")
    print("="*80)
...
def submit_human_approval(state: CampaignState, decision: dict) -> CampaignState:
    ...
    if action == "approve":
        ...
        print("✅ HUMAN APPROVED - Proceeding to Publisher")
        ...
        print(f"⚠️  HUMAN REJECTED - Sending to {revision_target.upper()} for revision")
...
if __name__ == "__main__":
    print("="*80)
    print("HUMAN APPROVAL AGENT - HITL Implementation")
    print("="*80)
    print("\nThis agent pauses the workflow after Reviewer approval")
    print("and waits for human decision before proceeding to Publisher.")
    print("\nUsage:")
    print("  1. Workflow runs until reviewer approves")
    print("  2. human_approval_node() pauses and displays summary")
    print("  3. Human reviews the outputs")
    print("  4. Human calls submit_human_approval() with decision")
    print("  5. Workflow continues based on decision")
    print("="*80)
