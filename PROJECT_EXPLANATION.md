...
│   ├── src/
│   │   ├── index.ts                    # Express app setup and Socket.io initialization
│   │   ├── modules/
│   │   │   ├── campaigns/
│   │   │   │   ├── campaign.controller.ts
│   │   │   │   ├── campaign.model.ts    # Mongoose Campaign schema
│   │   │   │   ├── campaign.routes.ts   # Express endpoints for campaigns
│   │   │   │   └── campaign.types.ts
│   │   │   └── notifications/
│   │   │       └── notification.service.ts
│   │   └── utils/
│   │       ├── ai-client.ts            # Connects Express to Python API and Redis
│   │       └── constants.ts
│   └── package.json
│
...
### 2. Backend (`/backend`)
*   **Tech Stack:** Node.js, Express, TypeScript, MongoDB (via Mongoose).
*   **Purpose:** The orchestrator and data store. Manages user authentication, project/campaign metadata, stores the final generated campaign states, manages WebSocket connections to the frontend for real-time updates, and triggers the Python AI service.
...
### 1. Database Model (`campaign.model.ts`)
We store the campaign state using Mongoose. The schema maps exactly to the fields generated by the Python LangGraph:
```typescript
const CampaignSchema = new Schema({
  ...
  status: { type: String, enum: ['draft', 'generating', 'pending_human_review', 'completed', 'failed'], default: 'draft' },
  
  // Storage for multi-agent results
  researchData: { type: Object },
  strategyData: { type: Object },
  copywritingData: { type: Object },
  imagePromptData: { type: Object },
  reviewData: { type: Object },
  contentCalendar: { type: Object },
  
  humanFeedback: { type: String },
  humanRevisionTarget: { type: String }
}, { timestamps: true });
```

### 2. Python Bridge (`ai-client.ts`)
The Express server communicates with the Python service using two mechanisms:
*   **REST API Requests:** When a user starts a campaign, Express sends a POST request to the Python service (`http://localhost:8000/api/campaign/generate`).
*   **Redis Event Streaming (Pub/Sub):** The Python service publishes live progress events to Redis. `ai-client.ts` runs a Redis subscriber that listens for these events and forwards them to the client over WebSockets:
    ```typescript
    import { createClient } from 'redis';
    
    const redisSubscriber = createClient({ url: process.env.REDIS_URL });
    
    export async function initRedisSubscriber(io: any) {
      await redisSubscriber.connect();
      
      // Listen to campaign progress updates from Python
      await redisSubscriber.subscribe('campaign_events', (message) => {
        const event = JSON.parse(message);
        
        // Broadcast the event to the specific client via Socket.io
        io.to(event.campaign_id).emit('campaign_progress', event);
      });
    }
    ```
